...
def _vcs_run(ctx):
    ...
    for (arg, custom_file_name) in ctx.attr.output_files.items():
        custom_file = ctx.actions.declare_file(custom_file_name)
        args.append("+" + arg + "=" + custom_file.path)
        outputs.append(custom_file)
...
vcs_run = rule(
    ...
    attrs = {
        ...
        "output_files": attr.string_dict(
            doc = "Files generated by simulation to keep",
        ),
        "run_env": attr.label(
